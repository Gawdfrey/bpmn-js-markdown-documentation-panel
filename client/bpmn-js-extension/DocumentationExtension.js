...
export default class DocumentationExtension {
  ...
  _updateSidebarPosition() {
    ...
    if (propertiesPanel) {
      ...
      this._sidebar.style.height = `${window.innerHeight - Math.max(panelTop, 0)}px`;
    } else {
  ...
  _setupResizeObserver() {
    ...
    if (propertiesPanel && window.ResizeObserver) {
      ...
      const rightPanel = document.querySelector('.djs-properties-panel-parent') ||
    ...
    const updatePosition = () => {
      if (this._sidebar && this._sidebar.classList.contains('visible') && this._sidebar.style.display !== 'none') {
        this._updateSidebarPosition();
      }
  ...
  _updatePreview() {
    ...
    if (markdown && markdown.trim()) {
      ...
      preview.innerHTML = "<p><em>No documentation yet. Start typing to see a preview.</em></p>";
    }
  }
  ...
  _checkCurrentView() {
    ...
    if ((xmlEditor && xmlEditor.offsetParent !== null) || 
...
DocumentationExtension.$inject = ["eventBus", "elementRegistry", "modeling", "moddle"];